...
test_passed(){ __log "PASS" "$GREEN" "✓ $*"; ((++TESTS_PASSED)); }
test_failed(){ __log "FAIL" "$RED"   "✗ $*"; ((++TESTS_FAILED)); }
...
cleanup() {
  ...
  # Delete test pods
  microk8s kubectl delete pod busy-dontwrite -n "$TEST_NS" --ignore-not-found=true --wait=false || true
  microk8s kubectl delete pod ubuntu-custom-profile -n "$TEST_NS" --ignore-not-found=true --wait=false || true
...
EOF
  ...
  local pod2_status=$(microk8s kubectl get pod ubuntu-custom-profile-2 -n "$TEST_NS" -o jsonpath='{.status.phase}' 2>/dev/null || echo "NotFound")
  log_info "New pod status: $pod2_status"

  if [ "$pod2_status" = "Running" ]; then
    log_warn "New pod started despite profile being removed (unexpected)"
  ...
  microk8s kubectl delete pod ubuntu-custom-profile-2 -n "$TEST_NS" --wait=false || true
}
